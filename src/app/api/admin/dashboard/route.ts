...
export async function GET() {
  try {
    ...
    const todayTrips = todayTripsData.reduce((sum, trip) => sum + trip.ordersCount, 0);
    const todayRevenue = todayTripsData.reduce((sum, trip) => sum + (trip.unitPrice * trip.ordersCount), 0);
    ...
    const monthRevenue = monthTripsData.reduce((sum, trip) => sum + (trip.unitPrice * trip.ordersCount), 0);
